...
if ($conn->connect_error) {
    ...
    $items = [];
    $query = "SELECT w.id, w.`title`, w.`description`, w.`url`, w.`category_id`, w.`album_id`, w.`created`, a.`title` as albumTitle, a.`description` as albumDescription FROM works as w INNER JOIN albums as a ON w.album_id = a.id WHERE w.category_id = " . $categoryId;
